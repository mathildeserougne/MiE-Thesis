...
################################################################################
## D.3 DiD avec effets fixes par année et par produit ########################
################################################################################

# Préparation des données : on garde 2013-2019 (pré) et 2021-2023 (post)
did_fixed_effects_data <- plot_data_eurozone_norm %>%
  filter(year %in% c(2013:2019, 2021:2023)) %>%
  mutate(
    treated = ifelse(product_type == "exposed_index", 1, 0),  # 1 = exposé
    post = ifelse(year >= 2021, 1, 0),                          # 1 = période post-2021
    did = treated * post                                        # Interaction (effet DiD)
  )

# Création de variables factorielles pour les effets fixes
did_fixed_effects_data$year_factor <- factor(did_fixed_effects_data$year)
did_fixed_effects_data$product_type_factor <- factor(did_fixed_effects_data$product_type)

# Régression DiD avec effets fixes (formule : outcome ~ treated + post + did + year_fe + product_fe)
lm_did_fe <- lm(
  index_value ~ treated + post + did + year_factor + product_type_factor,
  data = did_fixed_effects_data
)

# Affichage des résultats
summary_did_fe <- summary(lm_did_fe)
print("Difference-in-Differences avec effets fixes (année + produit) :")
print(summary_did_fe)

# Extraction du coefficient DiD (intérêt principal)
did_coef <- tidy(lm_did_fe) %>%
  filter(term == "did")
print(paste("Coefficient DiD (effet causal) :", round(did_coef$estimate, 3),
            "| p-value :", round(did_coef$p.value, 3)))

# Interprétation :
# - Le coefficient "did" capture l'effet moyen du traitement sur les produits exposés post-2021,
#   NET des effets fixes année (chocs macro communs) et produit (hétérogénéité groupe).
# - Si p-value < 0.05 : effet significatif au seuil 5%.


# Graphique
# 1. Prépare les prédictions du modèle D.3 (effets fixes)
plot_data_did_fe <- did_fixed_effects_data %>%
  mutate(
    predicted = predict(lm_did_fe),  # Prédictions du modèle
    product_type = factor(product_type,
                          levels = c("exposed_index", "nonexposed_index"),
                          labels = c("Exposed (Treated)", "Non-Exposed (Control)"))
  )

# 2. Graphique
ggplot(plot_data_did_fe, aes(x = year, y = index_value, color = product_type, group = product_type)) +
  # Données observées (points + lignes en pointillées)
  geom_point(size = 3) +
  geom_line(linetype = "dashed", size = 1) +  # Lignes pointillées pour les données brutes
  
  # Prédictions du modèle (lignes pleines)
  geom_line(aes(y = predicted), size = 1, linetype = "solid") +
  
  # Ligne verticale pour 2021 (traitement)
  geom_vline(xintercept = 2021, linetype = "dashed", color = "red", linewidth = 1) +
  ...
  scale_color_manual(
    name = "Product type",
    values = c("Exposed (Treated)" = "orange", "Non-Exposed (Control)" = "blue"),
    labels = c("Exposed (Treated)", "Non-Exposed (Control)")
  ) +
  
  # Annotations
  labs(
    title = "Difference-in-Differences with Year & Product Fixed Effects",
    subtitle = paste("DiD effect: +", round(did_coef$estimate, 1),
                     " (p =", round(did_coef$p.value, 3), ")"),
    x = "Year",
    y = "Index (Base 100 = 2013)"
  ) +
